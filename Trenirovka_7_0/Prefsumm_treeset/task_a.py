def get_treeset(array):
    ...
    while 2 ** degree_two < len(array):
        degree_two += 1
    size = 2 ** degree_two
    tree_set = [(0, 0)] * (size * 2 - 1)  # (максимум, количество максимумов)
    for i in range(len(array)):
        tree_set[i + size - 1] = (array[i], 1)
    for j in range(size - 2, -1, -1):
        ...
        if left_child[0] > right_child[0]:
            ...
            tree_set[j] = (left_child[0], left_child[1] + right_child[1])

    return tree_set, size


def get_cnt_max(l, r, tree, sec_l, sec_r, root=0):
    if l <= sec_l and r >= sec_r:
        ...
        res_left = get_cnt_max(l, r, tree, left_child_l, left_child_r, root * 2 + 1)
        right_child_l = (sec_l + sec_r) // 2 + 1
        right_child_r = sec_r
        res_right = get_cnt_max(l, r, tree, right_child_l, right_child_r, root * 2 + 2)
...
for _ in range(k):
    ...
    res = get_cnt_max(left - 1, right - 1, tree_cnt_max, 0, n_ - 1)
    print(*res)
