...
# несколько вагонов и снова часть оказавшихся вагонов вывезти в сторону пути 2. И так далее (так, что
# каждый вагон может лишь один раз заехать с пути 1 в тупик, а затем один раз выехать из тупика на путь 2).
# Заезжать в тупик с пути 2 или выезжать из тупика на путь 1 запрещается. Нельзя с пути 1 попасть на путь
# 2, не заезжая в тупик.
...
n = int(input())
v_s = [int(x) for x in input().split()]
stack = []
tek = 1
for v in v_s:
    stack.append(v)
    while len(stack) > 0 and stack[-1] == tek:
        stack.pop()
        tek += 1
