...
def check_for_rows(n, m, table, scores: dict):
    worst = 1
    res_row = 0
    res_column = 0
    for i in range(n):
...
def check_for_columns(n, m, table, scores: dict):
    worst = 1
    res_row = 0
    res_column = 0
    for j in range(m):
...
for _ in range(int(fast_input())):
    ...
    table_score = []
    score_dict = {x: 0 for x in range(1, 6)}
    for i in range(n):
        ...
        for j in range(m):
            score_dict[row[j]] += 1
    if n > m:
        row, column = check_for_columns(n, m, table_score, score_dict)
    else:
        row, column = check_for_rows(n, m, table_score, score_dict)
