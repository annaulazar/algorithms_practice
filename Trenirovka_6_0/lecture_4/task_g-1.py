...
def dfs_and_circle(root: int):
    visited[root] = 1
    stack = [(friend, root, 0) for friend in birds[root]]
    while stack:
        friend, prev, pre_prev = stack.pop()
        if friend != pre_prev and visited[friend] == 1:
            return True
        if visited[friend]:
            birds[prev].remove(friend)
        else:
            visited[friend] = 1
            for next_friend in birds[friend]:
                stack.append((next_friend, friend, prev))
    return False
